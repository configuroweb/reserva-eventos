	...
	function update_settings_info()
	{
		...
		if (isset($_FILES['img']) && $_FILES['img']['tmp_name'] != '') {
			...
			if (!in_array($type, $allowed)) {
				$resp['msg'] .= " Fallo al cargar la imagen por un formato inválido";
			} else {
				$new_height = 200;
				$new_width = 200;
				...
				if ($gdImg) {
					...
					$resp['msg'] .= " Fallo al cargar la imagen por un motivo desconocido";
				}
			}
		...
		$flash = $this->set_flashdata('success', 'Información del sistema actualizada correctamente');
		if ($update && $flash) {
			// var_dump($_SESSION);
			return true;
		}
